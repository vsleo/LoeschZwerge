...
- Name: Quiz_Brandschutz.vue >> Antwortmöglichkeiten sind vorhanden und auswählbar
- Location: C:\Users\erwin\IdeaProjects\LoeschZwerge\FrontEnd\tests\quizBrandschutz.spec.js:28:3
...
Locator: locator('.quiz-answer, .answer, [data-testid="quiz-answer"]').first()
Expected: visible
...
Call log:
  ...
  - waiting for locator('.quiz-answer, .answer, [data-testid="quiz-answer"]').first()
    ...
    at C:\Users\erwin\IdeaProjects\LoeschZwerge\FrontEnd\tests\quizBrandschutz.spec.js:35:35
```
...
```ts
   ...
   2 |
   3 | test.describe("Quiz_Brandschutz.vue", () => {
   4 |   test.beforeEach(async ({ page }) => {
   5 |     // Passe ggf. den Pfad an, falls deine Route anders ist
   6 |     await page.goto("http://localhost:4321/LoeschZwerge/quiz/brandschutz");
   7 |   });
   8 |
  ...
  18 |   });
  19 |
  20 |   test("Fragen des Brandschutz-Quiz werden angezeigt", async ({ page }) => {
